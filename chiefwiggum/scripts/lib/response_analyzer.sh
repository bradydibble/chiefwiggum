...
analyze_response() {
    ...
    if [[ "$output_format" == "json" ]]; then
        ...
        if parse_json_response "$output_file" ".json_parse_result" 2>/dev/null; then
            ...
            # Check for RALPH_STATUS block embedded in the result/summary text
            # Claude Code wraps output in JSON with the RALPH_STATUS block inside the "result" field
            if echo "$work_summary" | grep -q -- "---RALPH_STATUS---"; then
                local ralph_exit_sig=$(echo "$work_summary" | grep "EXIT_SIGNAL:" | cut -d: -f2 | xargs)
                local ralph_status=$(echo "$work_summary" | grep "STATUS:" | cut -d: -f2 | xargs)

                if [[ "$ralph_exit_sig" == "true" || "$ralph_status" == "COMPLETE" ]]; then
                    exit_signal="true"
                    has_completion_signal="true"
