...
extract_errors() {
    ...
    # Validate error_count before passing to jq --argjson
    error_count=$(validate_for_argjson "error_count" "$error_count" "number")
    ...
    if ! jq -n \
        ...
        '{category: $category, count: $count, details: $details}' 2>/tmp/jq_error_detect_$$.log; then
        echo "ERROR: jq failed in detect_errors. error_count='$error_count'" >&2
        cat /tmp/jq_error_detect_$$.log >&2
        rm -f /tmp/jq_error_detect_$$.log
        # Return minimal valid JSON
        echo '{"category":"unknown","count":0,"details":[]}'
    fi
    rm -f /tmp/jq_error_detect_$$.log
}
...
on_exit() {
    ...
    if [[ $exit_code -ne 0 ]]; then
        log_status "ERROR" "💀 Ralph loop exiting with code $exit_code"
        ...
        # CRITICAL: Write status before exit to prevent race conditions
        # Spawner will trust this status instead of overwriting with "crashed"
        local loop_val=${loop_count:-0}
        local calls_val=$(cat "$CALL_COUNT_FILE" 2>/dev/null || echo "0")

        # Determine crash category based on exit code
        local crash_message="Script error"
        if [[ $exit_code -eq 2 ]]; then
            crash_message="Script error (command failed)"
        elif [[ $exit_code -eq 130 ]]; then
            crash_message="Interrupted (SIGINT)"
        elif [[ $exit_code -eq 143 ]]; then
            crash_message="Terminated (SIGTERM)"
        fi

        update_status "$loop_val" "$calls_val" "unexpected_exit" "crashed" \
            "exit_code_$exit_code" \
            '{"category":"script_error","count":1,"details":[]}' \
...
main() {
    ...
    # passes the centralized ~/.chiefwiggum/ralphs/status/{ralph_id}.json path
    ...
    while true; do
        ...
        if [[ -n "$RALPH_ID" ]]; then
            ...
            if [[ -n "$current_task_id" && "$current_task_id" != "null" ]]; then
                ...
                if [[ "$task_status" == "completed" ]]; then
                    log_status "WARN" "⚠️ Current task already completed"

                    # In single-task mode, exit instead of claiming next
                    if [[ "$SINGLE_TASK_MODE" == "true" ]]; then
                        log_status "INFO" "🎯 Single-task mode: Exiting (task already completed)"
                        reset_session "single_task_already_complete"
                        break
                    fi
        ...
        if [[ "$test_progress" != "null" && -n "$test_progress" ]]; then
            # Validate test_progress is valid JSON before using with --argjson
            if echo "$test_progress" | jq empty 2>/dev/null; then
                # We have test results - wrap in progress_data structure
                if ! progress_data_json=$(jq -n \
                    ...
                    }' 2>/tmp/jq_error_progress_$$.log); then
                    echo "ERROR: jq failed to build progress_data. test_progress validation failed" >&2
                    cat /tmp/jq_error_progress_$$.log >&2
                    rm -f /tmp/jq_error_progress_$$.log
                    progress_data_json="null"
                else
                    rm -f /tmp/jq_error_progress_$$.log
                fi
            else
                echo "WARN: test_progress is not valid JSON, setting to null" >&2
                progress_data_json="null"
            fi
        else
            progress_data_json="null"
